...
auth_email="email@demo.com"                # The email used to login 'https://dash.cloudflare.com'
auth_method="global"                                 # Set to "global" for Global API Key or "token" for Scoped API Token
auth_key=""    # Your API Token or Global API Key
zone_identifier=""  # Can be found in the "Overview" tab of your domain
record_name="demo.com"                                      # Which record you want to be synced
...
if [[ ! $ip =~ ^$ipv4_regex$ ]]; then
    echo -s "DDNS Updater: Failed to find a valid IP."
    exit 2
fi
...
if [[ $record == *"\"count\":0"* ]]; then
  echo -s "DDNS Updater: Record does not exist, perhaps create one first? (${ip} for ${record_name})"
  exit 1
fi
